\begin{tikzpicture}
    ...
    \coordinate (firstAxle1) at (0,0);
    \coordinate (tireLeft1) at (0,{(\wTract-0.4)/2});
    \coordinate (tireRight1) at (0,{-(\wTract-0.4)/2});
    \fill[black] (firstAxle1) circle (0.05);

    % Draw a dashed line between tireLeft and tireRight
    \draw[dashed] (tireLeft1) -- (tireRight1);
    % Rotated Tires
    ...
    % Some locations
    \node [below right=0.05cm and 0.05cm of CoG1] {$\CG_1$};
    \node [below =0.1cm of CoG2] {$\CG_2$};
    \node [below =0.02cm of Coup] {$\Cr$};
    \node [below = 0.01cm of firstAxle1] {$P_1$};
    % Articulation Angle
    \draw[dashed] (Coup) to (Coup2);
    \draw[dashed] (Coup) to (Coup3);
    \draw[dashed] (Coup) to (rearTrail);
    \draw pic["$\psi_{2\to1}^z$",draw=black,angle radius=40*\scale,angle eccentricity=1.7*sqrt(\scale)] {angle=Coup3--Coup--Coup2};   

    % Coupling Force
    \draw[force] (Coup) --++ (1.5*\scale,0) node[above=0] {$f^x_{1,\Cr}$};
    \draw[force] (Coup) --++ (0,2*\scale) node[right=0] {$f^y_{1,\Cr}$};

    % Velocity in CoG
    \draw[force] (CoG1) --++ (1*\scale,0) node[above=0] {$v^x_{1,\CG_1}$};
    \draw[force] (CoG1) --++ (0,1.7*\scale) node[right=\nodeDist] {$v^y_{1,\CG_1}$};

    % Velocity in CoG
    \draw[force] (rearTrail) --++ ({1.5*\scale*cos(\ang)},{1.5*\scale*sin(\ang)}) node[above=0] {$v^x_{2,\CG_2}$};
    \draw[force] (rearTrail) --++ ({-1.7*\scale*sin(\ang)},{1.7*\scale*cos(\ang)}) node[right=0] {$v^y_{2,\CG_2}$};
    ...
    % Distance between first axle and CoG of the trailer
    \draw[<->] (firstAxle) -- (tireRight3) node[midway, below=-0.1] {$\bm{r}_{2,P_2\to\A3r_2}$};
    \draw[<->,dashed] (firstAxle) -- (thirdAxle) node[midway, above={\nodeDist}, right=\nodeDist] {${r}^x_{2,P_2\to\A3r_2}$};
    \draw[<->,dashed] (thirdAxle) -- (tireRight3) node[above right=\nodeDist] {};
